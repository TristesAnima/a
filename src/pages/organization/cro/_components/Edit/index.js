...
import { saveCRO, validateCRO } from '@/services/cro';
...
const Index = (props) => {
  ...
  const save = async (newData, form) => {
    ...
    const validateResult = await validateCRO(params);
    ...
    const saveResult = await saveCRO(params);
    messagePro({
      response: saveResult,
      mode: 'save',
      onSuccess: () => {
                                ...
                                affiliationName: {
                                  ...
                                  'x-validator': [
                                    ...
                                    (value, rule, ctx) => {
                                      let flag = true;
                                      form
                                        .query('croAffiliationList.*.affiliationName')
                                ...
                                affiliationNameEn: {
                                  ...
                                  'x-validator': [
                                    ...
                                    (value, rule, ctx) => {
                                      let flag = true;
                                      form
                                        .query('croAffiliationList.*.affiliationNameEn')
                                        .forEach((field, address) => {
                                          if (
                                            !ctx.field.address.match(address.entire) &&
                                            ctx.field.value === field.value
