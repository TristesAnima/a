...
import { saveSponsorContacts, isUserRelation } from '@/services/contacts';
...
const Index = (props) => {
  ...
  const save = async (newData) => {
    ...
    const saveResult = await saveSponsorContacts(params);
    messagePro({
      response: saveResult,
      mode: 'save',
      onSuccess: () => {
    ...
    <DrawerFormily
      ...
      formProps={{
        effects: () => {
          ...
          onFormMount((form) => {
            form.setFieldState('sponsorId', {
              loading: true,
            });
            getSponsorsPro().then((response) => {
              if (response.success) {
                form.setFieldState('sponsorId', {
                  dataSource: response.data.map((item) => ({
                    label: getNameWithLanguage(item.name, item.nameEn),
                    value: item.id,
                    sponsorAffiliationList: item.sponsorAffiliationList,
                  })),
                  loading: false,
                });
              }
          ...
          onFieldReact('sponsorId', (field, form) => {
            const { value: sponsorId, dataSource: sponsorDataSource } = field;
            if (sponsorId && sponsorDataSource) {
              const { sponsorAffiliationList = [] } = sponsorDataSource.find(
                (item) => item.value === sponsorId
              );
              const { value: affiliationId } = form.getFieldState('affiliationId');
              form.setFieldState('affiliationId', {
                value: sponsorAffiliationList.some((item) => item.id === affiliationId)
                  ? affiliationId
                  : null,
                dataSource:
                  sponsorAffiliationList?.map((item) => ({
