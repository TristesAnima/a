...
import { saveCroContacts, isUserRelation } from '@/services/contacts';
import { FormGrid, FormItem, FormLayout, Input, Select, Radio } from '@formily/antd';
import { createSchemaField } from '@formily/react';
import { onFormMount, onFieldReact } from '@formily/core';
...
const Index = (props) => {
  ...
  const save = async (newData) => {
    ...
    if (!isEdit && params.userId) {
      const relationResult = await isUserRelation({ ...params, type: 1 });
    ...
    const saveResult = await saveCroContacts(params);
    messagePro({
      response: saveResult,
      mode: 'save',
      onSuccess: () => {
    ...
    <DrawerFormily
      ...
      formProps={{
        effects: () => {
          ...
          onFormMount((form) => {
            form.setFieldState('croCompanyId', {
              loading: true,
            });
            getCROsPro().then((response) => {
              if (response.success) {
                form.setFieldState('croCompanyId', {
                  dataSource: response.data.map((item) => ({
                    label: getNameWithLanguage(item.name, item.nameEn),
                    value: item.id,
                    croAffiliationList: item.croAffiliationList,
                  })),
                  loading: false,
                });
              }
          ...
          onFieldReact('croCompanyId', (field, form) => {
            const { value: croCompanyId, dataSource: croDataSource } = field;
            if (croCompanyId && croDataSource) {
              const { croAffiliationList = [] } = croDataSource.find(
                (item) => item.value === croCompanyId
              );
              const { value: affiliationId } = form.getFieldState('affiliationId');
              form.setFieldState('affiliationId', {
                value: croAffiliationList.some((item) => item.id === affiliationId)
                  ? affiliationId
                  : null,
                dataSource:
                  croAffiliationList?.map((item) => ({
                    label: getNameWithLanguage(item.affiliationName, item.affiliationNameEn),
                    value: item.id,
                  })) || [],
              });
            }
                  ...
                  grid: {
                    ...
                    properties: {
                      ...
                      affiliationJob: {
                        ...
                        enum: affiliationJobList.map((item) => ({
                          label: item.value,
                          value: item.code,
                        })),
                      },
                      chargeMan: {
                        type: 'number',
                        title: formatMessage({ id: 'contacts.负责人' }),
                        'x-decorator': 'FormItem',
                        'x-decorator-props': {
                          gridSpan: 2,
                        },
                        ...
                        enum: [
                          {
                            label: formatMessage({ id: '是' }),
                            value: 1,
                          },
                          {
                            label: formatMessage({ id: '否' }),
                            value: 0,
                          },
                        ],
                      },
